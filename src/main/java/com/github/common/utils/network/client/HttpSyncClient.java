 ...
 */
public class HttpSyncClient implements InitializingBean {
     ...
     * 连接池最大连接数,默认100
     */
    @Value("${http.sync.maxTotalPool:100}")
    private Integer maxTotalPool;

    /**
     * 每个主机的并发最大数,默认20
     */
    @Value("${http.sync.maxConPerRoute:20}")
    private Integer maxConPerRoute;

    /**
     * 设置等待数据超时时间默认5秒钟,单位秒
     */
    @Value("${http.sync.socketTimeout:5}")
    private Integer socketTimeout;

    /**
     * 连接池取连接的超时时间,单位秒,默认5秒
     */
    @Value("${http.sync.connectionRequestTimeout:5}")
    private Integer connectionRequestTimeout;

    /**
     * 连接超时,单位秒,默认5秒
     */
    @Value("${http.sync.connectTimeout:5}")
    ...
    public HttpSyncClient(){
        System.out.println("HttpSyncClient初始化...");
    }
