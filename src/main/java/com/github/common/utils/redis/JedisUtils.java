...
public class JedisUtils {
    ...
    /**
     * redis address, like {"redis://host:port/password/db","redis://host:port/password/db"}
     */
    private static String address;

    public static void init(String address) {
        if(StringUtils.isBlank(address)){
            throw new IllegalArgumentException("Redis地址不能为:"+address);
        }
    ...
    private static ShardedJedis getInstance() {
        if (shardedJedisPool == null) {
            try {
                if (INSTANCE_INIT_LOCL.tryLock(TIME_OUT, TimeUnit.SECONDS)) {
                    try {
                        if (shardedJedisPool == null) {
                            // JedisPoolConfig
    ...
    public static String setStringValue(String key, String value) {
        return setStringValue(key,value,Integer.MAX_VALUE);
    }
    ...
    public static String setObjectValue(String key, Object value) {
        return setObjectValue(key,value,Integer.MAX_VALUE);
    }
